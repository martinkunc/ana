...
def generate_grouped_bar_chart(all_metrics_by_dir, output_path="metrics_chart.png"):
    ...
    if max_time_val <= 0:
        max_time_val = 1.0
    ax_time.set_ylim(0, max_time_val * 1.15)  # 15% headroom
    ax_score.set_ylim(0, 100 * 1.08)          # 8% headroom
    ...
    for axis, bar, metric_name, value, is_score in label_entries:
        ...
        if is_score:
            text = f"{value:.0f}%"
            voffset = max_time_val * 0.01  # scale offset so it looks similar
        else:
    ...
    fig.subplots_adjust(top=0.90)  # extra top space
    fig.tight_layout()
...
def generate_network_requests_table(all_metrics_by_dir, output_path="network_requests.png"):
    ...
    n_rows = len(rows)
    fig_height = max(3, min(0.2 * n_rows + 1, 40))  # avoid too small / huge
